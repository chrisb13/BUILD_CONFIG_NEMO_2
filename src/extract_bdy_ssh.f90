...
REAL*4,ALLOCATABLE,DIMENSION(:,:,:)  :: sossheig_GLO, sossheig_bdy
  ...
  DO kmonth=1,12
    ...
    DO kday=1,31
      ...
      IF ( existfile ) THEN
        ...
        ALLOCATE( sossheig_GLO(mlon,mlat,mtime)  )
        ALLOCATE( time(mtime) )
        ...
        do j=1,mlat
        do l=1,mtime
          if ( .not. abs(sossheig_GLO(i,j,l)) .lt. 100.0 ) then
            sossheig_GLO(i,j,l) = 0.0
          endif
        enddo
        enddo
        enddo
        ...
        do kbdy=1,mxbt
           ...
           if ( aa .gt. eps .and. zjSt(nbit(kbdy,1),nbjt(kbdy,1)) .gt. 1 ) then
             ...
             do kt=1,mtime
                sossheig_bdy(kbdy,1,kt) = (   sossheig_GLO( ziWt(nbit(kbdy,1),nbjt(kbdy,1)), zjNt(nbit(kbdy,1),nbjt(kbdy,1)), kt ) * aNW   &
                &                           + sossheig_GLO( ziWt(nbit(kbdy,1),nbjt(kbdy,1)), zjSt(nbit(kbdy,1),nbjt(kbdy,1)), kt ) * aSW   &
                &                           + sossheig_GLO( ziEt(nbit(kbdy,1),nbjt(kbdy,1)), zjNt(nbit(kbdy,1),nbjt(kbdy,1)), kt ) * aNE   &
                &                           + sossheig_GLO( ziEt(nbit(kbdy,1),nbjt(kbdy,1)), zjSt(nbit(kbdy,1),nbjt(kbdy,1)), kt ) * aSE ) &
                &                         * tmask_REG(nbit(kbdy,1),nbjt(kbdy,1),1) / aa
             enddo

           else
 
             sossheig_bdy(kbdy,1,:) = 0.e0
