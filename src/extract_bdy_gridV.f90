...
!
! Used to derive baroclinic velocities (V) along the BDY
!
! 0- Initialiartions
! 1a- Read GLOBAL mask
...
CHARACTER(LEN=150)                   :: file_coord, file_in_gridU, file_bdy_gridV,                   &
...
DO kday=1,31
  ...
  IF ( existfile ) THEN
    ...
    write(*,*) 'Reading V input dimensions in ', TRIM(file_in_gridV)
    ...
    if (status .ne. 0) status = NF90_INQ_DIMID(fidV,"z",dimID_depthv)
    ...
    status = NF90_INQ_VARID(fidV,"depthv",depth_ID)
    if ( status .ne. 0 ) status = NF90_INQ_VARID(fidV,"depth",depth_ID)
    if ( status .ne. 0 ) status = NF90_INQ_VARID(fidV,"deptht",depth_ID)
    if ( status .ne. 0 ) status = NF90_INQ_VARID(fidV,"nav_lev",depth_ID)
    if ( status .ne. 0 ) status = NF90_INQ_VARID(fidV,"z",depth_ID)
    ...
    status = NF90_GET_VAR(fidV,depth_ID,depthv)                  ; call erreur(status,.TRUE.,"getvar_depthv")
  ...
  DO kmonth=1,12
    ...
    DO kday=1,31
      ...
      IF ( existfile ) THEN
        ...
        status = NF90_OPEN(TRIM(file_in_gridU),0,fidU)                ; call erreur(status,.TRUE.,"read GLOBAL U") 
        ...
        status = NF90_OPEN(TRIM(file_in_gridV),0,fidV)                ; call erreur(status,.TRUE.,"read GLOBAL V") 
        ...
        do kbdy=1,mxbv
          ...
          !  NB: here no need to account for vvl because vbcl = v3d - sum_k(v3d*e3v)/sum_k(e3v)
          !                                                   = v3d - sum_k(v3d*e3v_0)/sum_k(e3v_0)
          !                                      because e3v = e3v_0 * ( 1 + ssh_v / H )
          do kt=1,mtime
            tmpdep = 0.e0
            tmpbtp = 0.e0
            do kz=1,mdepthv                   
        ...
        DEALLOCATE( vomecrty_GLO, vozocrtx_GLO_V )
