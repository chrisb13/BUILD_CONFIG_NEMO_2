...
!
! Used to extract T,S along the BDY
!
! 0- Initialiartions
! 1a- Read GLOBAL mask
...
! 5- Process all gridT files over specified period
! 5a- Read input temperature
! 5b- Read input salinity
! 5c- Remove possible NaNs
! 5d- Fill values on bdyT
! 5e- Convert to conservative temperature if needed
! 5f- Write BDY netcdf file
!
...
DO kday=1,31
  ...
  do kfmt=1,size(list_fmt)
     ...
     SELECT CASE(nfmt)
        CASE(191)
          write(file_in_gridT,191) TRIM(data_dir), kyear, TRIM(data_prefix), kyear, TRIM(sep1), kmonth, TRIM(sep2), kday, TRIM(data_suffix_T)
        CASE(192)
          write(file_in_gridT,192) TRIM(data_dir), kyear, TRIM(data_prefix), kyear, TRIM(sep1), kmonth, TRIM(data_suffix_T) 
        CASE(193)
          write(file_in_gridT,193) TRIM(data_dir), TRIM(data_prefix), kyear, TRIM(sep1), kmonth, TRIM(sep2), kday, TRIM(data_suffix_T)
        CASE(194)
          write(file_in_gridT,194) TRIM(data_dir), TRIM(data_prefix), kyear, TRIM(sep1), kmonth, TRIM(data_suffix_T)
        CASE DEFAULT 
          write(*,*) 'wrong nfmt value >>>>>> stop !'
          stop
     END SELECT
     write(*,*) 'Looking for existence of ', TRIM(file_in_gridT)
  ...
  DO kmonth=1,12
    ...
    DO kday=1,31
      ...
      SELECT CASE(nfmt)
        CASE(191)
          write(file_in_gridT,191) TRIM(data_dir), kyear, TRIM(data_prefix), kyear, TRIM(sep1), kmonth, TRIM(sep2), kday, TRIM(data_suffix_T)
          write(file_in_gridS,191) TRIM(data_dir), kyear, TRIM(data_prefix), kyear, TRIM(sep1), kmonth, TRIM(sep2), kday, TRIM(data_suffix_S)
        CASE(192)
          write(file_in_gridT,192) TRIM(data_dir), kyear, TRIM(data_prefix), kyear, TRIM(sep1), kmonth, TRIM(data_suffix_T) 
          write(file_in_gridS,192) TRIM(data_dir), kyear, TRIM(data_prefix), kyear, TRIM(sep1), kmonth, TRIM(data_suffix_S) 
        CASE(193)
          write(file_in_gridT,193) TRIM(data_dir), TRIM(data_prefix), kyear, TRIM(sep1), kmonth, TRIM(sep2), kday, TRIM(data_suffix_T)
          write(file_in_gridS,193) TRIM(data_dir), TRIM(data_prefix), kyear, TRIM(sep1), kmonth, TRIM(sep2), kday, TRIM(data_suffix_S)
        CASE(194)
          write(file_in_gridT,194) TRIM(data_dir), TRIM(data_prefix), kyear, TRIM(sep1), kmonth, TRIM(data_suffix_T)
          write(file_in_gridS,194) TRIM(data_dir), TRIM(data_prefix), kyear, TRIM(sep1), kmonth, TRIM(data_suffix_S)
        CASE DEFAULT 
          write(*,*) 'wrong nfmt value >>>>>> stop !'
          stop
      END SELECT
